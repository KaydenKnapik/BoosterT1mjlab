...
def gated_height_reward(
  env: ManagerBasedRlEnv,
  desired_height: float,
  asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
  root_asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
  """Height reward multiplied by orientation — no credit for headstands."""
  asset: Entity = env.scene[asset_cfg.name]
  root_asset: Entity = env.scene[root_asset_cfg.name]
  height = asset.data.body_link_pos_w[:, asset_cfg.body_ids, 2].squeeze(-1)
  clamped = torch.clamp(height, max=desired_height)
  height_r = (torch.exp(clamped) - 1.0) / (math.exp(desired_height) - 1.0)
  gravity = root_asset.data.projected_gravity_b
  up = _UP_VEC.to(gravity.device)
  error = torch.sum(torch.square(up - gravity), dim=-1)
  orientation_factor = torch.exp(-2.0 * error)
  return height_r * orientation_factor
